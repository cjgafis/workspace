...
public class Curso
{
	...
	public int rangoConMasNotas ( ) 
	{
		...
		for (int i = 0; i<notas.length; i++)
		{
			if (notas[i] >= 0.0 && notas[i] <= 1.99 )
			{
				rango1 ++;
			}	
			
			else if (notas[i] >= 2.0 && notas[i] <= 3.49 )
			{
				rango2 ++;
			}	
			
			else if (notas[i] >= 3.5 && notas[i] <= 5 )
			{
				rango3 ++;
			}	
		}
		...
		return retorno; // el retorno es provisional.
	}
	...
	public int sumadasDanElNumero (double numero ) 
	{
		...
		boolean llego = false;
		int retorno = -1;
		for (int i = 0; i<notas.length && llego == false; i++)
		{
			valor = valor + notas[i];
			contador ++;
			if (valor == numero)
	...
	{
		return "El numero del rango es: " +  rangoConMasNotas ( );
	}
