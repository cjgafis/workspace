...
public class Curso
{
	...
	public int rangoConMasNotas ( ) 
	{
		...
		for(int i=0; i<notas.length; i++) 
		{
			...
			if(notaActual>=0 && notaActual<=1.99) 
			{
				CantidadRango1++;
			}
			if(notas[i]>=2 && notas[i]<=3.49) {
				CantidadRango2++;
			}
			if(notas[i]>=3.5 && notas[i]<=5) {
				CantidadRango3++;
			}
		}
		if(CantidadRango1>CantidadRango2 && CantidadRango1>CantidadRango3) 
		{
			resultado = 1;
		}
		if(CantidadRango2>CantidadRango3 && CantidadRango2>CantidadRango1) 
		{
			resultado = 2;
		}
		
		if(CantidadRango3>CantidadRango1 && CantidadRango3>CantidadRango2) 
		{
			resultado = 3;
		}
	...
	public int sumadasDanElNumero (double numero ) 
	{
		...
		for(int i=0; i<notas.length && total<numero; i++)	
		{
			double actual = notas[i];
		...
		return resultado; // el retorno es provisional.
	...
	{
		return "el rango mas repetido es: " +  rangoConMasNotas();
	}
	...
	public String metodo2( )
	{
		...
		return "el numero de notas necesarias para llegar al numero ingresado es" + sumadasDanElNumero(valor);
	}
}