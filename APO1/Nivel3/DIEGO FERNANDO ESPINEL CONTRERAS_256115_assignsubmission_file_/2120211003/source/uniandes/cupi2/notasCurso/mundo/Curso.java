...
public class Curso
{
	...
	public int rangoConMasNotas ( ) 
	{
		...
		for(int i=0 ; i<notas.length; i++)
		{
			...
			if (notas[i] >= 0 && notas[i]<= 1.9)
				{
					rango1 ++;
				}
			else if (notas[i] > 2 && notas[i]<= 3.49)
				{
					rango2 ++;
				}
			else  if (notas[i]>3.5)
		...
		// El uso es para saber en cual rango tienen mas notas
		if (rango1>rango2 || rango1>rango3)
			{
				respuesta = 1;
			}
		else if (rango2>rango3)
			{
				respuesta = 2;
			}
		else 
	...
	public String metodo1( )
	{
		...
		return  "El Rango que mayor notas tiene es el rango  # " + rangoConMasNotas ( ) ;
	}
