...
public class Curso
{
	 ...
	 */
	public double rangoConMasNotas ( )	
	{		
		int rango1 = 0;
		int rango2 = 0;
		int rango3 = 0;
		int rangoConMasNotas = 0;
		...
		for (int i = 0; i < notas.length; i++) 
		{
			...
			else if (notas[i]>=2.0 && notas[i]<3.5)
			{
				rango2++;
			}
			else if (notas[i]>=3.5 && notas[i]<=5.0 )
			{
				rango3++;
			}	
		}
	...
	public int sumadasDanElNumero (double numero ) 
	{
		...
		for(int i=0; i<notas.length && resul<numero; i++)	
		{
	...
	{	
		return "Rango con más notas: " +rangoConMasNotas ( );
	}
	...
	{
		double valor= 0.0;
		valor=Double.parseDouble(JOptionPane.showInputDialog("ingrese un valor"));
		return "Se necesitan " + sumadasDanElNumero (valor ) + " notas" ; 
	}
}