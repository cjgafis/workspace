...
public class Curso
{
	...
	public int rangoConMasNotas ( ) 
	{
		...
		for (int i = 0; i < notas.length ; i++ ) 
		{
			if (notas[i] >= 0.0 && notas[i] < 2.0 ) 
			{
				rango1 = rango1 + 1;
			}
			if (notas [i] >= 2.0 && notas [i] < 3.5) 
			{
				rango2 = rango2 + 1;	
			}	
			if (notas [i] >= 3.5 && notas[i] < 5.1) 
			{
				rango3= rango3 + 1; 
			}
		}
	...
	public int sumadasDanElNumero (double numero ) 
	{
		...
		for (int i = 0; i < notas.length && (sumatoria < numero) ; i++) 
		{
	...
	{
		return "El rango que posee la mayor cantidad de notas es:" + rangoConMasNotas() ;
	}
	...
	{
		double numero = Double.parseDouble(JOptionPane.showInputDialog("Ingrese el valor"));

		return " El numero de notas necesarios para llegar al numero " + "" +  numero + " es: " + sumadasDanElNumero(numero);
