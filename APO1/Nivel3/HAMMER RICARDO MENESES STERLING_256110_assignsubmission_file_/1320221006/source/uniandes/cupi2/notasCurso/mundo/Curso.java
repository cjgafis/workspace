...
public class Curso
{
	...
	public int rangoConMasNotas ( ) 
	{
		...
		for(int i = 0; i < notas.length-1; i++) 
		{
			...
			{
				if(notas[i] > notas[j] && j>i && notas[i] <=1.99) 
				{
					rango = 1;
				}
				else if(notas[i] > notas[j] && j>i && notas[i] <=3.49) 
				{
					rango = 2;
				}
				else if(notas[i] > notas[j] && j>i && notas[i] <=5.0) 
				{
					rango = 3;
				}
			}
		}
		return rango; // el retorno es provisional.
	}
	...
	public int sumadasDanElNumero (double numero ) 
	{
		...
		for(int i = 0; i < notas.length; i++) 
		{
			if(faltante < numero || faltante == numero) 
			{
				faltante = faltante + notas[i];
				suma++;
			}
			else 
			{
	...
	public String metodo1( )
	{
		...
		int metodo = cursoUno.rangoConMasNotas(); 
		return "Rango " + metodo;
	}
	...
	public String metodo2( )
	{
		...
		int metodo = cursoDos.sumadasDanElNumero(valor);
		return "La notas sumadas para llegar al numero son: " + metodo ; 
	}
}