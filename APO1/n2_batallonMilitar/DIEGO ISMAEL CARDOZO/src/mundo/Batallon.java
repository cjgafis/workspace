...
public class Batallon 
{
	...
	public Batallon() 
	{
		s1 = new Soldado(123, "Celso", "Rodriguez", 19, 1.65,2);
		s2 = new Soldado(234, "Carlos", "Ramirez", 20, 1.69,3);
		s3 = new Soldado(345, "Pablo", "Acero", 17, 1.73,1);
	...
	public Soldado darmenosantiguo( )
	 {
	
	    	Soldado menosantiguo = null;
	    	
	    	if ( s1.calcularantiguedad() < s2.calcularantiguedad() &&
	    	     s1.calcularantiguedad() < s3.calcularantiguedad() &&
	    	     s1.calcularantiguedad() < s4.calcularantiguedad() )
	    	    
	    	{
	    		menosantiguo = s1;
	    	}
	    	else if ( s2.calcularantiguedad() < s1.calcularantiguedad() &&
		    	     s2.calcularantiguedad() < s3.calcularantiguedad() &&
		    	     s2.calcularantiguedad() < s4.calcularantiguedad() )
		    	    
		    	{
		    		menosantiguo = s2;
		    	}
	    	else if ( s3.calcularantiguedad() < s1.calcularantiguedad() &&
		    	     s3.calcularantiguedad() < s2.calcularantiguedad() &&
		    	     s3.calcularantiguedad() < s4.calcularantiguedad() )
		    	    
		    	{
		    		menosantiguo = s3;
		    	}
	    	
	    	else  { menosantiguo = s4;
	...
	public Soldado darmasantiguoque()
	
	      int anos = 3;
	      
	      if (s1.masAntiguoQue() == true)
	      {	  
	    	  anos++;
	...
	public static void main(String[] args) 
	{
		...
		System.out.println("El soldado menos antiguo es: " + baser10.darmenosantiguo(  ));
